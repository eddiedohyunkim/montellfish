...
var hrAngle = 360 - hr * 15 - (min * 3 / 12),
    minAngle = min * 6 + (sec * 6 / 60),
    secAngle = 360 - sec * 6 + (milsec * 0.36 / 1000),
    dateAngle = 360 - date * 12 + 12,
    star1Angle = 360 - hr * 30 - (min * 6 / 12),
    star2Angle = min * 12 + (sec * 6 / 60),
    star3Angle = 360 - sec * 3 + (milsec * 0.36 / 1000),
    star4Angle = hr * 15 - (min * 3 / 12);
...
setAngle("day-wrapper", dateAngle);
setAngle("min-wrapper", minAngle);
setAngle("sec-wrapper", secAngle);
setAngle("star1-wrapper", star1Angle);
setAngle("star2-wrapper", star2Angle);
setAngle("star3-wrapper", star3Angle);
setAngle("star4-wrapper", star4Angle);